...
if($range == "bulanan"){
    ...
    $GLOBALS["lis"] = query("SELECT * FROM pemesanan WHERE tanggal BETWEEN '$bulannow-01' AND '$bulannow-31' ORDER BY id desc");
}else if($range == "tahunan"){
    $datenow = date_create(date("Y-m-d"));
    $tahunnow = date_format($datenow, 'Y');
    $GLOBALS["lis"] = query("SELECT * FROM pemesanan WHERE tanggal BETWEEN '$tahunnow-01-01' AND '$tahunnow-12-31' ORDER BY id desc");
}
$list = $GLOBALS["lis"];
?>
<!DOCTYPE html>
